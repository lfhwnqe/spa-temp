...
describe('formatAddress', () => {
  ...
  describe('error handling', () => {
    it('should throw AddressFormatError for non-string input', () => {
      expect(() => formatAddress(null as unknown as string)).toThrow(
        new AddressFormatError('Address must be a string'),
      );
      expect(() => formatAddress(undefined as unknown as string)).toThrow(
        new AddressFormatError('Address must be a string'),
      );
      expect(() => formatAddress(123 as unknown as string)).toThrow(
        new AddressFormatError('Address must be a string'),
      );
    });
